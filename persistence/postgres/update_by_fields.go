...
func (r *PostgresRepository[E, M]) UpdateByFields(cc *customctx.CustomContext, id string, fields map[string]any) result.Result[E] {
	if id == "" {
	...
	// Actualizar el item en la base de datos usando GORM
	if err := r.Connection.Save(&updatedModel).Error; err != nil {
		dbErr := ErrorDatabaseOperation.WithCause(err)
		cc.Logger().Error(dbErr.Error())
		cc.AddError(dbErr)
		return result.Err[E](dbErr)
